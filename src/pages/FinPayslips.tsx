...
async function exportPayslipsExcel(rows: PayslipRow[], month: number, year: number): Promise<void> {
  ...
  summary.addRow([
    ...
    'Adjustment Type', 'Adjustment Amount', 'Adjustment Reason', 'Expense Total', 'Net Pay',
  ]);
  for (const r of rows) {
    summary.addRow([
      ...
      r.expenseTotal, r.netPay,
    ]);
  }
  summary.addRow([]);
  summary.addRow([
    'TOTAL', '', '', '', '', rows.reduce((s, r) => s + r.proratedSalary, 0), '', '', '',
    rows.reduce((s, r) => s + r.expenseTotal, 0), rows.reduce((s, r) => s + r.netPay, 0),
  ]);
