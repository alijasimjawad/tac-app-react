...
// Maps ZXing BarcodeFormat numeric enum values to their canonical uppercase names.
// Mirrors @zxing/library's BarcodeFormat enum so ZXing produces the same
// symbology strings as the native BarcodeDetector path ("DATA_MATRIX", "CODE_128", …).
// Exported for unit testing — not part of the public scanner API.
export const ZXING_FORMAT_NAMES: Record<number, string> = {
  0:  'AZTEC',
  1:  'CODABAR',
  2:  'CODE_39',
  3:  'CODE_93',
    ...
    try {
      ...
      const controls = await reader.decodeFromVideoElement(videoEl, (result, _err) => {
        if (result && !this.paused) {
          ...
          const fmt    = ZXING_FORMAT_NAMES[fmtNum] ?? String(fmtNum);
